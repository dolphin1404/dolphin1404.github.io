...
- 저장소 언어 필터
- 이름, 이메일, 메시지를 받는 Contact 폼과 Formspree 이메일 전송
...
- `768px`: 햄버거 메뉴를 일반 메뉴로 바꾸고 Hero의 소개와 프로필 사진, Contact를 2열로 배치
...
Contact 폼은 빈 값과 이메일 형식을 확인합니다. 오류는 입력란 아래에 표시하고 `aria-invalid`도 함께 변경합니다. 검증을 통과하면 Formspree로 메시지를 전송하며, 전송 중에는 버튼을 잠그고 성공 또는 실패 상태를 안내합니다.
